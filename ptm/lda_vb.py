...
class vbLDA:
    ...
    def __init__(
        self, n_doc, n_voca, n_topic, alpha=0.1, beta=0.01, is_compute_bound=True
    ):
        self.n_voca = n_voca
        ...
        self._lambda = 1 * np.random.gamma(
            100.0, 1.0 / 100, (self.n_topic, self.n_voca)
        )
        self._Elogbeta = dirichlet_expectation(self._lambda)
        self._expElogbeta = np.exp(self._Elogbeta)
        self.gamma_iter = 5
        self.gamma = 1 * np.random.gamma(100.0, 1.0 / 100, (self.n_doc, self.n_topic))

        self.is_compute_bound = is_compute_bound

    def fit(self, doc_ids, doc_cnt, max_iter=100, verbose: bool = True):
        ...
        for iter in xrange(max_iter):
            ...
            if verbose:
                logger.info(
                    "[ITER] %d,\telapsed time:%.2f,\tELBO:%.2f",
                    iter,
                    time.time() - tic,
                    bound,
                )
    ...
    def approx_bound(self, doc_ids, doc_cnt, gamma):
        ...
        score = score + np.sum(
            gammaln(self.beta * self.n_voca) - gammaln(np.sum(self._lambda, 1))
        )
